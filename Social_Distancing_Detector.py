...
vs =cv2.VideoCapture( r"newvideo2ytbe.mp4"if "newvideo2ytbe.mp4"else 0)#r"pedestrian.mp4"if "pedestrian.mp4"else
writer =None
        ...
        for i in range(len(D)):
            for j in range(len(D)):
                if (D[i][3]-D[i][1]) >= (D[j][3]-D[j][1]):
                    if (D[i][3]-D[i][1])*0.66<=(D[j][3]-D[j][1])<=(D[i][3]-D[i][1]):
                    ...
                    if (D[j][3]-D[j][1])*0.66<=(D[i][3]-D[i][1])<=(D[j][3]-D[j][1]):
                        if (D[i][2]-D[i][0]) >= (D[j][2]-D[j][0]):
                            if (D[j][2]-D[j][0])*0.3<=Cendis[i,j]<=(D[i][2]-D[i][0])*1.4: #(D[j][2]-D[j][0])<=
                                violate.add(i)
                                violate.add(j)
